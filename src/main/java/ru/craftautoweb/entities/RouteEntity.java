...
*/
    ...
    public Integer Save() {
        ...
        if(id == null) {
            ...
            RouteEntity updatedItem = getById(id);
    ...
    public static RouteEntity getById(int id) {
        // Получаем объект сесси для работы с базой
        Session session = HibernateUtil.getSession();

        // формируем запрос
        String sql = "from RouteEntity where id=:id";

        // и выполняем его
        List<RouteEntity> list = session.createQuery(sql).
                setParameter("id", id).
                list();
        session.close();
    ...
    public static void Delete(int id) {
        ...
        // Обновляем запись
        RouteEntity item = (RouteEntity)session.get(RouteEntity.class, new Integer(id));
        if(item != null) {
